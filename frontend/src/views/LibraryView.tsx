...
import { Bot, ChevronDown, ChevronLeft, ChevronRight, Download, Eye, FileSpreadsheet, FileText, Folder, FolderOpen, Globe2, Grid2X2, List, Menu, Plus, Search, Upload, X, Check, MoreVertical, Edit2, RefreshCw, Trash2 } from "lucide-react";
import { createCategory, createDownloadUrl, createPreviewUrl, createSubject, deleteDocument, fetchCategories, fetchDocument, fetchExtractionStatus, fetchLibraryDocuments, fetchSubjects, formatFileSize, retryExtraction, updateSubject, deleteSubject, updateCategory, updateDocumentVisibility, deleteCategory, type ApiExtractionStatus, type CategoryItem, type SubjectItem } from "../api/documents.api";
...
function uniqueTaxonomyItems<T extends { id: string; name: string }>(items: T[]) {
  return sortTaxonomyItems([...new Map(items.map((item) => [item.id, item])).values()]);
}

function upsertTaxonomyItem<T extends { id: string; name: string }>(items: T[], item: T) {
  return uniqueTaxonomyItems([...items.filter((current) => current.id !== item.id), item]);
}
...
function getPreviewFrameUrl(result: { url: string; contentType?: string; fallbackToOfficeViewer?: boolean }) {
  return shouldUseOfficeViewer(result) ? `https://view.officeapps.live.com/op/embed.aspx?src=${encodeURIComponent(result.url)}` : result.url;
}

function getFullPreviewUrl(result: { url: string; contentType?: string; fallbackToOfficeViewer?: boolean }) {
  return shouldUseOfficeViewer(result) ? `https://view.officeapps.live.com/op/view.aspx?src=${encodeURIComponent(result.url)}` : result.url;
}
...
export function LibraryView() {
  ...
  const [publishingDocumentIds, setPublishingDocumentIds] = useState<Set<string>>(new Set());
  ...
  async function handleDeleteSelectedDocuments() {
    ...
    const confirmMsg = ids.length === 1 ? text("Bạn có chắc chắn muốn xóa tài liệu này không?", "Are you sure you want to delete this document?") : text(`Bạn có chắc chắn muốn xóa ${ids.length} tài liệu đã chọn không?`, `Are you sure you want to delete ${ids.length} selected documents?`);
  ...
  async function handlePublishDocument(document: LibraryDocument) {
    ...
    try {
      const publishedDocument = await updateDocumentVisibility(document.id, "PUBLIC");
      setDocuments((current) => current.map((item) => (item.id === document.id ? publishedDocument : item)));
      setPreviewDocument((current) => (current?.id === document.id ? publishedDocument : current));
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : text("Không thể công khai tài liệu.", "Could not make the document public."));
    } finally {
  ...
  async function handleRetryExtraction(document: LibraryDocument) {
    ...
    setDocuments((current) => current.map((item) => (item.id === document.id ? { ...item, indexStatus: "PROCESSING" } : item)));
    ...
    try {
      ...
      throw new Error(text("Phân tích AI vẫn đang xử lý. Hãy kiểm tra lại sau ít phút.", "AI analysis is still processing. Check again in a few minutes."));
    } catch (error) {
      if (!retryStarted) {
        setDocuments((current) => current.map((item) => (item.id === document.id && item.indexStatus === "PROCESSING" ? { ...item, indexStatus: "FAILED" } : item)));
      }
      setErrorMessage(error instanceof Error ? error.message : text("Không thể chạy lại phân tích AI.", "Could not retry AI analysis."));
    } finally {
    ...
    <main id="main-content" className="library-page">
      <header className="library-page-heading">
        <div>
          ...
          <h1>{text("Kiến thức của bạn, được sắp xếp rõ ràng.", "Your knowledge, organized for deeper work.")}</h1>
          <p>{text("Duyệt theo môn học và danh mục, theo dõi trạng thái AI hoặc tiếp tục đặt câu hỏi.", "Browse by subject and category, track AI readiness, or continue asking questions.")}</p>
        </div>
      </header>

      <div className="library-shell">
        <button type="button" className="library-folder-mobile-toggle" onClick={() => setFolderOpen(true)}>
          <Menu size={17} />
        ...
        <aside className={`library-folder-panel${folderOpen ? " open" : ""}`}>
          <div className="library-folder-header">
            <div>
              <p className="eyebrow">{text("CẤU TRÚC LIBRARY", "LIBRARY STRUCTURE")}</p>
              <strong>{text("Thư mục học tập", "Study folders")}</strong>
            </div>
            <button type="button" className="icon-button library-folder-close" onClick={() => setFolderOpen(false)}>
              <X size={18} />
            </button>
          </div>
          <button type="button" className={`library-folder-root${!subjectId ? " active" : ""}`} onClick={() => selectFolder()}>
            <FolderOpen size={18} />
          ...
          <div className="folder-group-heading">
            ...
            <button type="button" onClick={() => setAddingSubject((value) => !value)}>
              <Plus size={14} />
          ...
          {addingSubject ? (
            <div className="folder-quick-add">
              ...
              <input value={newSubjectCode} onChange={(event) => setNewSubjectCode(event.target.value)} placeholder={text("Mã", "Code")} />
              <button type="button" onClick={handleCreateSubject} disabled={isCreating || !newSubjectName.trim()}>
                {text("Lưu", "Save")}
              </button>
            </div>
          ) : null}
          ...
          <div className="folder-group-heading" style={{ marginTop: "8px" }}>
            ...
            <button type="button" onClick={() => setAddingCategory((value) => !value)}>
              <Plus size={14} />
              {text("Thêm nhanh", "Quick add")}
            </button>
          </div>
          {addingCategory ? (
            <div className="folder-quick-add folder-quick-add--category">
              ...
              <button type="button" onClick={handleCreateCategory} disabled={isCreating || !newCategoryName.trim()}>
                {text("Lưu", "Save")}
              </button>
            </div>
          ) : null}
            ...
            {subjects.map((subject) => {
              ...
              return (
                <div className="folder-subject" key={subject.id}>
                  {isEditingSubject ? (
                    <form onSubmit={(e) => handleUpdateSubject(subject.id, e)} className="folder-rename-form" onClick={(e) => e.stopPropagation()}>
                      <input type="text" value={editSubjectName} onChange={(e) => setEditSubjectName(e.target.value)} autoFocus className="rename-input" placeholder={text("Tên môn học", "Subject name")} />
                      <input type="text" value={editSubjectCode} onChange={(e) => setEditSubjectCode(e.target.value)} className="rename-code-input" placeholder={text("Mã", "Code")} />
                    ...
                    <div className={`folder-subject-row${active ? " active" : ""}`}>
                      <button type="button" className="folder-expand" onClick={() => toggleSubject(subject.id)} aria-expanded={expanded}>
                      ...
                      <div className="folder-action-container" onClick={(e) => e.stopPropagation()}>
                        <button type="button" className={`folder-actions-trigger${activeMenu?.type === "subject" && activeMenu?.id === subject.id ? " active" : ""}`} onClick={(e) => handleMenuToggle(e, "subject", subject.id)} title={text("Thao tác", "Actions")}>
                      ...
                      {loadedCategories.length === 0 ? (
                        ...
                        loadedCategories.map((category) => {
                          ...
                          return isEditingCategory ? (
                            <form key={category.id} onSubmit={(e) => handleUpdateCategory(category.id, e)} className="folder-rename-form folder-rename-form--category" onClick={(e) => e.stopPropagation()}>
                              <input type="text" value={editCategoryName} onChange={(e) => setEditCategoryName(e.target.value)} autoFocus className="rename-input" placeholder={text("Tên danh mục", "Category name")} />
                              <button type="submit" className="folder-action-confirm-btn" title={text("Lưu", "Save")}>
                                <Check size={14} />
                              </button>
                              <button type="button" onClick={cancelEdit} className="folder-action-cancel-btn" title={text("Hủy", "Cancel")}>
                            ...
                            <div key={category.id} className={`folder-category-row${isCategoryActive ? " active" : ""}`}>
                              <button type="button" className={`folder-category-select${isCategoryActive ? " active" : ""}`} onClick={() => selectFolder(subject.id, category.id)}>
                                <FileText size={14} />
                                <span>{category.name}</span>
                              </button>
                              <div className="folder-action-container" onClick={(e) => e.stopPropagation()}>
                                <button type="button" className={`folder-actions-trigger${activeMenu?.type === "category" && activeMenu?.id === category.id ? " active" : ""}`} onClick={(e) => handleMenuToggle(e, "category", category.id)} title={text("Thao tác", "Actions")}>
                                  <MoreVertical size={14} />
                                </button>
        ...
        <section className="library-content">
          <section className="library-controls">
            <div className="library-controls-row-1">
              <label className="library-search">
                <Search size={18} />
                <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={text("Tìm theo tiêu đề, mô tả hoặc tên tệp...", "Search title, description, or file name...")} />
              </label>
            ...
            <div className="library-breadcrumb">
              ...
              {selectedSubject ? (
                <>
                  ...
                  <button type="button" onClick={() => selectFolder(selectedSubject.id)}>
            ...
            <div className="library-controls-row-2">
              <div className="library-filters-scroll-container">
                <div className="library-filters">
                  ...
                  >
                    <option value="">{text("Tất cả trạng thái AI", "All AI statuses")}</option>
                    <option value="COMPLETED">{text("AI sẵn sàng", "AI ready")}</option>
                    <option value="PROCESSING">{text("Đang xử lý", "Processing")}</option>
                    <option value="PENDING">{text("Đang chờ", "Pending")}</option>
                  ...
                  >
                    <option value="">{text("Tất cả quyền hiển thị", "All visibility")}</option>
          ...
          {errorMessage ? (
            <div className="library-api-error" role="alert">
              <strong>{text("Không thể hoàn tất yêu cầu", "The request could not be completed")}</strong>
              <p>{errorMessage}</p>
          ...
          <div className="library-result-count">
            ...
            {someSelected && (
              <span className="library-selection-info">
                ...
                <button type="button" className="delete-selected-btn" onClick={handleDeleteSelectedDocuments} disabled={isDeleting}>
                  <Trash2 size={14} />
          ...
          {isLoading ? (
            ...
            <div className="soft-empty-state library-empty">
              ...
              <p>{text("Thử thư mục khác, xóa bộ lọc hoặc tải tài liệu mới.", "Try another folder, clear filters, or upload a new document.")}</p>
            </div>
          ) : view === "table" ? (
                      ...
                      <td className="library-table-document">
                        <div className="library-document-cell">
                          ...
                          <span>
                            ...
                            <small>
                              {document.fileType} · {formatFileSize(document.fileSize)} · {document.visibility === "PRIVATE" ? text("riêng tư", "private") : text("công khai", "public")} · {document.moderationStatus === "APPROVED" ? text("đã duyệt", "approved") : document.moderationStatus === "REJECTED" ? text("bị từ chối", "rejected") : text("chờ duyệt", "pending review")}
                            </small>
            ...
            <section className="library-card-grid">
              {documents.map((document) => (
                <article className={`library-document-card${selectedDocIds.has(document.id) ? " card-selected" : ""}`} key={document.id}>
                  ...
                  <div>
                    ...
                    <p>{document.description || text("Chưa có mô tả.", "No description yet.")}</p>
                  </div>
          ...
          {pagination.totalPages > 1 ? (
            <nav className="library-pagination" aria-label={text("Phân trang tài liệu", "Document pagination")}>
              <button type="button" onClick={() => setPage((value) => value - 1)} disabled={page <= 1}>
                <ChevronLeft size={16} />
                {text("Trước", "Previous")}
              </button>
              <span>{text(`Trang ${page} / ${pagination.totalPages}`, `Page ${page} of ${pagination.totalPages}`)}</span>
              <button type="button" onClick={() => setPage((value) => value + 1)} disabled={page >= pagination.totalPages}>
                {text("Sau", "Next")}
      ...
      {previewDocument ? (
        <div className="preview-overlay" role="presentation" onMouseDown={() => setPreviewDocument(undefined)}>
          <article className="preview-dialog" role="dialog" aria-modal="true" aria-label={`${text("Xem tài liệu", "View document")} ${previewDocument.title}`} onMouseDown={(event) => event.stopPropagation()}>
            <header>
              <div>
                <span className="document-type-icon">
            ...
            <div className="preview-document-sheet">
              {isPreviewLoading ? (
                ...
                <div className="preview-frame-state preview-frame-state--error">
                  <strong>{text("Không thể hiển thị bản xem trước", "Preview unavailable")}</strong>
                  <p>{previewError}</p>
                  <button type="button" className="secondary-button" onClick={() => void openObject(previewDocument, "preview")}>
                    <Eye size={16} />
            ...
            <div className="preview-details">
              ...
              <p>{previewDocument.description || text("Chưa có mô tả.", "No description yet.")}</p>
            ...
            <footer>
              ...
              <button type="button" className="secondary-button" onClick={() => void openObject(previewDocument, "download")}>
                <Download size={16} />
                {text("Tải xuống", "Download")}
              </button>
              {previewDocument.indexStatus === "READY" ? (
                <Link href={`${ROUTES.aiChat}?scope=document&document=${previewDocument.id}`} className="primary-button">
                  <Bot size={16} />
                  {text("Hỏi AI", "Ask AI")}
                </Link>
              ) : (
...
export function DocumentActions({ document, text, isRetrying, isPublishing, onPreview, onDownload, onRetry, onPublish, onUnpublish, onDelete }: { document: LibraryDocument; text: (vi: string, en: string) => string; isRetrying: boolean; isPublishing: boolean; onPreview: () => void; onDownload: () => void; onRetry: () => void; onPublish: () => void; onUnpublish: () => void; onDelete: () => void }) {
  ...
  return (
    <div className="document-actions">
      <button type="button" title={text("Xem", "View")} aria-label={text(`Xem ${document.title}`, `View ${document.title}`)} onClick={onPreview}>
        <Eye size={16} />
      </button>
      <button type="button" title={text("Tải xuống", "Download")} aria-label={text(`Tải ${document.title}`, `Download ${document.title}`)} onClick={onDownload}>
        <Download size={16} />
      </button>
      {document.visibility === "PRIVATE" ? (
        <button type="button" className="publish-document-action" title={text("Công khai tài liệu", "Make document public")} aria-label={text(`Công khai ${document.title}`, `Make ${document.title} public`)} onClick={onPublish} disabled={isPublishing}>
          <Globe2 size={16} />
      ...
      <button type="button" className="delete-document-action" title={text("Xóa", "Delete")} aria-label={text(`Xóa ${document.title}`, `Delete ${document.title}`)} onClick={onDelete}>
        <Trash2 size={16} />
      </button>
    </div>
  );
