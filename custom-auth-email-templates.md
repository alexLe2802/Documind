...
## Goal
Send branded registration-verification and password-reset emails from the NestJS backend using Firebase Admin action links and configurable SMTP accounts.

## Tasks
- [x] Add SMTP environment validation and a reusable mail module. Verify: disabled delivery fails explicitly and enabled delivery uses SMTP.
...
- [x] The backend is ready to send from `registration@documind.icu` and `reset-password@documind.icu` after SMTP credentials are supplied.
- [x] Automated tests, lint, and production builds pass.

## Notes
- Cloudflare DNS does not create mailboxes or provide SMTP credentials.
